...
dealSchema.methods.getDealEditInfo = function () {
  const openingHours = this.opening.map(row => {
    ...
    newRow.day = {
      string: convertDayEnumToString(row.day),
      enum: row.day // newRow.open = `${row.open.toString().slice(0, -2)}:${row.open.toString().slice(-2)}`
      // newRow.close = `${row.close.toString().slice(0, -2)}:${row.close.toString().slice(-2)}`

    };
    newRow.open = row.open.toString().length <= 2 ? `0:${row.open.toString().slice(-2).padStart(2, '0')}` : `${row.open.toString().slice(0, -2)}:${row.open.toString().slice(-2)}`;
